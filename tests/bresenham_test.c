...
// Test fill callback context (static for single-threaded test)
static struct {
    bool **bits;
    size_t w, h;
    bool bad_range;
...
static void test_fill_callback_ref(int8_t y, int8_t x_left, int8_t x_right) {
    ...
    for (int x = x_left; x < x_right; x++) {
        if (x >= 0 && (size_t)x < test_fill_context.w) {
...
static void test_fill_callback_impl(int8_t y, int8_t x_left, int8_t x_right) {
    // Same implementation
    test_fill_callback_ref(y, x_left, x_right);
}
...
extern void graphics_fill_triangle_ref(struct graphics_screen_pos a, struct graphics_screen_pos b, struct graphics_screen_pos c);
extern void graphics_fill_triangle_impl(struct graphics_screen_pos a, struct graphics_screen_pos b, struct graphics_screen_pos c);
...
static bool test_combo_ref(const bres_test_t *test, 
    ...
    test_fill_context.bits = bits;
    test_fill_context.w = test->w;
    test_fill_context.h = test->h;
    test_fill_context.bad_range = false;
    
    graphics_fill_triangle_ref(a, b, c);
...
static bool test_combo_impl(const bres_test_t *test, 
    ...
    bool **ref_bits = malloc(test->h * sizeof(bool *));
    for (size_t i = 0; i < test->h; i++) {
        ref_bits[i] = calloc(test->w, sizeof(bool));
    }
    
    test_fill_context.bits = ref_bits;
    test_fill_context.bad_range = false;
    
    graphics_fill_triangle_ref(a, b, c);
    
    // Build reference result string
    char *ref_result = build_result_string(ref_bits, test->w, test->h);
